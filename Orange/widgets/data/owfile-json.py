...
from Orange.data.io import FileFormat, UrlReader, JsonReader, UrlJsonReader
...
class OWFile(widget.OWWidget, RecentPathsWComboMixin):
    ...
    recent_urls = Setting(["http://data.police.uk/api/forces/leicestershire"])
    recent_xpath = Setting(["engagement_methods"])
    ...
    def __init__(self):
        ...
        file_button.setIcon(self.style().standardIcon(QtGui.QStyle.SP_DirOpenIcon))
        ...
        self.sheet_box.layout().addWidget(self.sheet_label, QtCore.Qt.AlignLeft)
        self.sheet_box.layout().addWidget(self.sheet_combo, QtCore.Qt.AlignVCenter)
        ...
        # xpath
        rb_button = gui.widgetLabel(vbox, "XPath:", addToLayout=False)
        layout.addWidget(rb_button, 5, 0, QtCore.Qt.AlignVCenter)

        self.xpath_combo = xpath_combo = QtGui.QComboBox()
        xpath_model = NamedURLModel(self.sheet_names)
        xpath_model.wrap(self.recent_xpath)
        xpath_combo.setModel(url_model)
        xpath_combo.setSizePolicy(Policy.MinimumExpanding, Policy.Fixed)
        xpath_combo.setEditable(True)
        xpath_combo.setInsertPolicy(url_combo.InsertAtTop)
        url_edit = url_combo.lineEdit()
        l, t, r, b = url_edit.getTextMargins()
        url_edit.setTextMargins(l + 5, t, r, b)
        layout.addWidget(xpath_combo, 5, 1, 3, 3)
        xpath_combo.activated.connect(self._xpath_set)
    ...
    def load_data(self):
        ...
        with catch_warnings(record=True) as warnings:
            try:
                data = self.reader.read(self.xpath_combo.currentText())
    ...
    def _get_reader(self):
        ...
        if self.source == self.LOCAL_FILE:
            reader = JsonReader(self.last_path()) #FileFormat.get_reader(self.last_path())
            return reader
        elif self.source == self.URL:
            return UrlJsonReader(self.url_combo.currentText())
